...
class Sales extends CI_Controller
{
    ...
    public function addSales()
    {
        ...
        try {
            ...
            $sales = array(
                ...
                'SaleMaster_ReturnAmount'        => $data->sales->returnTotal,
            ...
            // SALE RETURN PART START

            if (count($data->returnCart) > 0) {
                $salesReturn = array(
                    'SaleMaster_InvoiceNo'    => $invoice,
                    'SaleReturn_ReturnDate'   => $data->sales->salesDate,
                    'SaleReturn_ReturnAmount' => $data->sales->returnTotal,
                    'SaleReturn_Description'  => $data->sales->returnNote,
                    'Status'                  => 'a',
                    'AddBy'                   => $this->session->userdata("FullName"),
                    'AddTime'                 => date('Y-m-d H:i:s'),
                ...
                foreach ($data->returnCart as $product) {
                    $returnDetails = array(
                    ...
                    if ($inventoryCount == 0) {
                        $inventory = array(
                            ...
                            'sales_return_quantity' => $product->quantity,
    ...
    public function getSales()
    {
        ...
        if (isset($data->salesId) && $data->salesId != 0 && $data->salesId != '') {
            ...
            $salesReturn = $this->db->query("SELECT srd.*,p.Product_Code,p.Product_Name
                FROM  tbl_salereturn sr
                left JOIN tbl_salesmaster sm on sr.SaleMaster_InvoiceNo = sm.SaleMaster_InvoiceNo
                left JOIN tbl_salereturndetails srd on srd.SaleReturn_IdNo = sr.SaleReturn_SlNo
                LEFT JOIN tbl_product p on p.Product_SlNo = srd.SaleReturnDetailsProduct_SlNo
                WHERE sm.SaleMaster_SlNo = ?
                and sm.Status = 'a'
            ", $data->salesId)->result();

            $res['salesReturn'] = $salesReturn;
        }
        $sales = $this->db->query("
            select 
            ...
            from tbl_saledetails sd
            where sd.SaleMaster_IDNo = sm.SaleMaster_SlNo
            and sd.Status = 'a') as qty
            from tbl_salesmaster sm
            left join tbl_customer c on c.Customer_SlNo = sm.SalseCustomer_IDNo
            left join tbl_employee e on e.Employee_SlNo = sm.employee_id
            left join tbl_brunch br on br.brunch_id = sm.SaleMaster_branchid
    ...
    public function updateSales()
    {
        ...
        try {
            ...
            $sales = array(
                'SalseCustomer_IDNo'             => $customerId,
                'SaleMaster_Bill_no'             => $data->sales->SaleMaster_Bill_no,
                'employee_id'                    => $data->sales->employeeId,
                'SaleMaster_SaleDate'            => $data->sales->salesDate,
                'SaleMaster_SaleType'            => $data->sales->salesType,
                'SaleMaster_TotalSaleAmount'     => $data->sales->total,
            ...
            $oldSaleReturnDetails = $this->db->query("select srd.* 
                from tbl_salesmaster sm
                left JOIN tbl_salereturn sr on sr.SaleMaster_InvoiceNo = sm.SaleMaster_InvoiceNo
                left JOIN tbl_salereturndetails srd on srd.SaleReturn_IdNo = sr.SaleReturn_SlNo
                where sm.SaleMaster_SlNo = ?", $salesId)->result();

            if (count($oldSaleReturnDetails) > 0) {
                foreach ($oldSaleReturnDetails as $rProduct) {
                    $this->db->query("delete from tbl_salereturndetails where SaleReturnDetails_SlNo = ?", $rProduct->SaleReturnDetails_SlNo);
                    ...
                    where product_id = ?
                    and branch_id = ?
                ", [$rProduct->SaleReturnDetails_ReturnQuantity, $rProduct->SaleReturnDetailsProduct_SlNo, $this->session->userdata('BRANCHid')]);
                }
            }

            if (count($data->returnCart) > 0) {

                $saleInvoice = $this->db->query("SELECT sm.SaleMaster_InvoiceNo,sr.SaleReturn_SlNo
                    FROM tbl_salesmaster sm
                    LEFT JOIN tbl_salereturn sr on sr.SaleMaster_InvoiceNo = sm.SaleMaster_InvoiceNo
                WHERE sm.SaleMaster_SlNo = ?", $salesId)->row();

                $salesReturn = array(
                    'SaleMaster_InvoiceNo'    => $saleInvoice->SaleMaster_InvoiceNo,
                    'SaleReturn_ReturnDate'   => $data->sales->salesDate,
                    'SaleReturn_ReturnAmount' => $data->sales->returnTotal,
                    'SaleReturn_Description'  => $data->sales->returnNote,
                    'Status'                  => 'a',
                ...
                $this->db->where('SaleReturn_SlNo', $saleInvoice->SaleReturn_SlNo);
                $this->db->update('tbl_salereturn', $salesReturn);

                foreach ($data->returnCart as $product) {
                    $returnDetails = array(
                        'SaleReturn_IdNo'                  => $saleInvoice->SaleReturn_SlNo,
                        'SaleReturnDetailsProduct_SlNo'    => $product->productId,
                        'SaleReturnDetails_ReturnQuantity' => $product->quantity,
                        'SaleReturnDetails_ReturnAmount'   => $product->total,
                        'Status'                           => 'a',
...
<?php
    ...
    public function  deleteSales()
    {
        ...
        try {
            ...
            //DELETE SALE RETURN START
            $returnCount = $this->db->query("select * from tbl_salereturn sr where sr.SaleMaster_InvoiceNo = ? and sr.Status = 'a'", $sale->SaleMaster_InvoiceNo)->row();

            if ($returnCount) {
                $returnDetails = $this->db->query("select * from tbl_salereturndetails srd where srd.SaleReturn_IdNo = ? and srd.Status = 'a'", $returnCount->SaleReturn_SlNo)->result();

                foreach ($returnDetails as $rProduct) {
                    $this->db->set('Status', 'd')->where('SaleReturnDetails_SlNo', $rProduct->SaleReturnDetails_SlNo)->update("tbl_salereturndetails");
            ...
            $this->db->set('Status', 'd')->where('SaleMaster_InvoiceNo', $sale->SaleMaster_InvoiceNo)->update("tbl_salereturn");
            //DELETE SALE RETURN END
